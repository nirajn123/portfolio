...
function checkCashRegister(price, cash, cid) {
  ...
  let change = [];
  let closedFlag = true;
  //Multi-dimmensional array needs a deepCopy.
  let cidOriginal = deepCopy(cid);
    ...
    //it is important that this is checked for the right type of bill
        let maxAvail = 0;
          for(let j=0;j<cid.length;j++){
            if(cid[j][0]===key) {
              maxAvail = cid[j][1];
              cid[j][1]-= (Math.min(calc*currency[key],maxAvail*100))/100;
            }
          }
        const obj =[];
        //now populate the change array
  ...
  if (remVal>0) {
    ...
    for (let k=0;k<cid.length;k++){
            if(cid[k][1]>0) closedFlag=false;
          }
      
      if(closedFlag&&remVal==0){ retObj.status = "CLOSED";
      retObj.change = [...cidOriginal];
      //console.log(cidOriginal);
      }
      else{ retObj.status = "OPEN";
      retObj.change = [...change];
      }
...
//Deep copy function for array with Object deep copy reference removed, which would be necessary if the multi-dimmensional array has Objects in it.
const deepCopy = (arr) => {
  let copy = [];
  arr.forEach(elem => {
